...
def comment(post_id):
    ...
    if error is not None:
        ...
        db = get_db()
        db.execute(
            'INSERT INTO comments (title, body, author_id, post_id)'
            ' VALUES (?, ?, ?, ?)', (title, body, g.user['id'], post_id))
        db.commit()
...
def get_comments(post_id):
    comments = get_db().execute(
        'SELECT p.id, title, body, created, author_id, username'
        ' FROM comments p JOIN user u ON p.author_id = u.id'
        ' WHERE p.post_id = ?', (post_id, )).fetchall()
    return comments
